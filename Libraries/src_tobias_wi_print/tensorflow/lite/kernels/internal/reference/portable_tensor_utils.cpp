...
#include "C:/Users/Sajjad/Documents/PlatformIO/Projects/firmware_helloprint/lib/tflite_micro_stock/third_party/gemmlowp/fixedpoint/fixedpoint.h"
...
void PortableSymmetricQuantizeFloats(const float* values, const int size,
  ...
  Serial.println("portable_tensor_utils.cpp1");
...
void PortableSymmetricQuantizeFloats(const float* values, const int size,
                                     int8_t* quantized_values, float min_value,
                                     float max_value, float* scaling_factor) {
  Serial.println("portable_tensor_utils.cpp2");
  const int32_t kScale = 127;
  const float range = std::max(abs(min_value), abs(max_value));
  if (range == 0) {
...
void PortableAsymmetricQuantizeFloats(const float* values, const int size,
  ...
  Serial.println("portable_tensor_utils.cpp3");
  ...
  if (rmin == rmax) {
    ...
    *offset = 0;
    ...
    const double zero_point_from_min_error =
        abs(qmin_double) + abs(rmin / scale);
    const double zero_point_from_max_error =
        abs(qmax_double) + abs(rmax / scale);
...
void PortableMatrixBatchVectorMultiplyAccumulate(const float* matrix,
  ...
  Serial.println("portable_tensor_utils.cpp4");
...
void PortableMatrixBatchVectorMultiplyAccumulate(
  ...
  Serial.println("portable_tensor_utils.cpp5");
...
void PortableMatrixBatchVectorMultiplyAccumulate(
  ...
  Serial.println("portable_tensor_utils.cpp6");
...
void PortableSparseMatrixBatchVectorMultiplyAccumulate1x4(
  ...
  Serial.println("portable_tensor_utils.cpp7");
...
void PortableSparseMatrixBatchVectorMultiplyAccumulate(
  ...
  Serial.println("portable_tensor_utils.cpp8");
...
void PortableSparseMatrixBatchVectorMultiplyAccumulate(
  ...
  Serial.println("portable_tensor_utils.cpp9");
...
void PortableMatrixBatchVectorMultiplyAccumulateImpl(
    ...
    T* output) {
  Serial.println("portable_tensor_utils.cpp10");
...
void PortableMatrixBatchVectorMultiplyAccumulate(
  ...
  Serial.println("portable_tensor_utils.cpp11");
...
void PortableMatrixBatchVectorMultiplyAccumulate(
  ...
  Serial.println("portable_tensor_utils.cpp12");
  PortableMatrixBatchVectorMultiplyAccumulateImpl(
      input, bias, input_to_gate_weights, multiplier, shift, n_batch, n_input,
      n_output, output_zp, output);
}
...
void PortableMatrixBatchVectorMultiply(const int8_t* input,
  ...
  Serial.println("portable_tensor_utils.cpp13");
...
void PortableMatrixBatchVectorMultiply(
  ...
  Serial.println("portable_tensor_utils.cpp14");
...
void PortableApplyLayerNorm(const int16_t* input,
  ...
  Serial.println("portable_tensor_utils.cpp15");
...
void PortableApplyLayerNormFloat(const int16_t* input,
                                 ...
                                 int16_t* output) {
  Serial.println("portable_tensor_utils.cpp16");
...
void PortableMatrixScalarMultiplyAccumulate(const int8_t* matrix,
  ...
  Serial.println("portable_tensor_utils.cpp18");
...
void PortableApplySigmoid(const int16_t* input, int32_t n_batch,
  ...
  Serial.println("portable_tensor_utils.cpp19");
  for (int batch = 0; batch < n_batch; ++batch) {
...
void PortableApplySigmoidFloat(const int16_t* input, int32_t n_batch,
                               int32_t n_input, int16_t* output) {
  Serial.println("portable_tensor_utils.cpp20");
  const int32_t int16_max = std::numeric_limits<int16_t>::max();
  const int32_t int16_min = std::numeric_limits<int16_t>::min();
  for (int batch = 0; batch < n_batch; ++batch) {
    for (int i = 0; i < n_input; ++i) {
...
void PortableApplyTanhImpl(const int16_t* input, int32_t n_batch,
                           int32_t n_input, int16_t* output) {
  Serial.println("portable_tensor_utils.cpp21");
...
void PortableApplyTanh(int32_t integer_bits, const int16_t* input,
  ...
  Serial.println("portable_tensor_utils.cpp22");
...
void PortableApplyTanhFloat(const int16_t* input, int32_t n_batch,
                            ...
                            int16_t* output) {
  Serial.println("portable_tensor_utils.cpp23");
  const int32_t int16_max = std::numeric_limits<int16_t>::max();
  const int32_t int16_min = std::numeric_limits<int16_t>::min();
...
void PortableCwiseMul(const int16_t* input_1, const int16_t* input_2,
  ...
  Serial.println("portable_tensor_utils.cpp24");
  for (int batch = 0; batch < n_batch; ++batch) {
    for (int i = 0; i < n_input; ++i) {
      const int index = batch * n_input + i;
...
void PortableCwiseMul(const int16_t* input_1, const int16_t* input_2,
  ...
  Serial.println("portable_tensor_utils.cpp25");
  for (int batch = 0; batch < n_batch; ++batch) {
    for (int i = 0; i < n_input; ++i) {
      const int index = batch * n_input + i;
      const int16_t a = input_1[index];
...
void PortableCwiseAdd(const int16_t* input_1, const int16_t* input_2,
                      int n_batch, int n_input, int16_t* output) {
  Serial.println("portable_tensor_utils.cpp26");
  for (int batch = 0; batch < n_batch; ++batch) {
    for (int i = 0; i < n_input; ++i) {
      const int index = batch * n_input + i;
...
float PortableVectorVectorDotProduct(const float* vector1, const float* vector2,
                                     int v_size) {
  Serial.println("portable_tensor_utils.cpp27");
  float result = 0.0;
...
inline int32_t VectorVectorDotProduct(const int16_t* vector1,
  ...
  Serial.println("portable_tensor_utils.cpp28");
  int32_t result = 0;
  for (int v = 0; v < v_size; v++) {
    result += *vector1++ * *vector2++;
  }
...
void PortableVectorBatchVectorCwiseProductAccumulate(
  ...
  Serial.println("portable_tensor_utils.cpp30");
  for (int b = 0; b < n_batch; b++) {
    for (int v = 0; v < v_size; v++) {
...
void PortableSub1Vector(const float* vector, int v_size, float* result) {
  Serial.println("portable_tensor_utils.cpp31");
  for (int v = 0; v < v_size; v++) {
    *result++ = 1.0f - *vector++;
  }
}
...
void PortableSub1Vector(const int16_t* vector, int v_size, int16_t* result) {
  Serial.println("portable_tensor_utils.cpp32");
...
void PortableVectorScalarMultiply(const int8_t* vector, const int v_size,
  ...
  Serial.println("portable_tensor_utils.cpp33");
  for (int v = 0; v < v_size; ++v) {
    *result++ = scale * *vector++;
  }
}
...
void PortableMeanStddevNormalization(const float* __restrict__ input_vector,
                                     ...
                                     int v_size, int n_batch) {
  Serial.println("portable_tensor_utils.cpp34");
  for (int batch = 0; batch < n_batch; ++batch) {
    float sum = 0.0f;
...
void PortableTwoGateSaturatingAdd(const int8_t* input, int8_t input_zp,
                                  ...
                                  int16_t* output) {
  Serial.println("portable_tensor_utils.cpp35");
  const int32_t int16_max = std::numeric_limits<int16_t>::max();
  const int32_t int16_min = std::numeric_limits<int16_t>::min();
