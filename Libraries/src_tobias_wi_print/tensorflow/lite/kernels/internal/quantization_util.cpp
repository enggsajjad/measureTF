...
void QuantizeMultiplier(double double_multiplier, int32_t* quantized_multiplier,
                        int* shift) {
  Serial.println("quantization_util.cpp1");
...
void QuantizeMultiplierGreaterThanOne(double double_multiplier,
  ...
  Serial.println("quantization_util.cpp2");
...
void QuantizeMultiplierSmallerThanOneExp(double double_multiplier,
                                         int32_t* quantized_multiplier,
                                         int* left_shift) {
  Serial.println("quantization_util.cpp3");
...
int64_t IntegerFrExp(double input, int* shift) {
  Serial.println("quantization_util.cpp4");
...
double DoubleFromFractionAndShift(int64_t fraction, int shift) {
  Serial.println("quantization_util.cpp5");
  union {
    double double_value;
    uint64_t double_as_uint;
  } result;
...
double IntegerDoubleMultiply(double a, double b) {
  Serial.println("quantization_util.cpp6");
  int a_shift;
...
int IntegerDoubleCompare(double a, double b) {
  Serial.println("quantization_util.cpp7");
  int a_shift;
  const int64_t a_fraction = IntegerFrExp(a, &a_shift);
  int b_shift;
  const int64_t b_fraction = IntegerFrExp(b, &b_shift);
...
void PreprocessSoftmaxScaling(double beta, double input_scale,
  ...
  Serial.println("quantization_util.cpp8");
...
void PreprocessLogSoftmaxScalingExp(double beta, double input_scale,
  ...
  Serial.println("quantization_util.cpp9");
...
int CalculateInputRadius(int input_integer_bits, int input_left_shift,
  ...
  Serial.println("quantization_util.cpp10");
#ifdef TFLITE_EMULATE_FLOAT
...
void NudgeQuantizationRange(const float min, const float max,
                            ...
                            float* nudged_scale) {
  Serial.println("quantization_util.cpp11");
...
void FakeQuantizeArray(const float nudged_scale, const float nudged_min,
  ...
  Serial.println("quantization_util.cpp12");
  // This code originates from tensorflow/core/kernels/fake_quant_ops_functor.h.
...
bool CheckedLog2(const float x, int* log2_result) {
  Serial.println("quantization_util.cpp13");
  ...
  *log2_result = static_cast<int>(x_log2_rounded);
  //return abs(x_log2_fracpart) < 1e-3f;
  return abs(x_log2_fracpart) < 1e-3f;
}
...
void QuantizeMultiplierArray(const double* effective_scales, size_t size,
                             ...
                             int* effective_shift) {
  Serial.println("quantization_util.cpp14");
