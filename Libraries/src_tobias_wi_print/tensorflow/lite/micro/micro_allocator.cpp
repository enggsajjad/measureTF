...
TfLiteStatus AllocationInfoBuilder::AddTensors(const SubGraph* subgraph,
                                               const int32_t* offline_offsets,
                                               TfLiteEvalTensor* eval_tensors) {
  Serial.println("micro_allocator.cpp1");
...
TfLiteStatus AllocationInfoBuilder::GetOfflinePlannedOffsets(
    const Model* model, const int32_t** offline_planner_offsets) {
  Serial.println("micro_allocator.cpp2");
  if (model->metadata()) {
...
TfLiteStatus AllocationInfoBuilder::AddScratchBuffers(
    internal::ScratchBufferRequest* scratch_buffer_requests,
    ScratchBufferHandle* scratch_buffer_handles) {
  Serial.println("micro_allocator.cpp3");
...
TfLiteStatus CreatePlan(ErrorReporter* error_reporter,
  ...
  Serial.println("micro_allocator.cpp4");
...
TfLiteStatus CommitPlan(ErrorReporter* error_reporter,
                        ...
                        const AllocationInfo* allocation_info,
                        size_t allocation_info_size) {
  Serial.println("micro_allocator.cpp5");
...
void* GetFlatbufferTensorBuffer(
    ...
    const flatbuffers::Vector<flatbuffers::Offset<Buffer>>* buffers) {
  Serial.println("micro_allocator.cpp6");
...
TfLiteStatus InitializeTfLiteTensorFromFlatbuffer(
  ...
  Serial.println("micro_allocator.cpp7");
...
TfLiteStatus InitializeTfLiteEvalTensorFromFlatbuffer(
  ...
  Serial.println("micro_allocator.cpp9");
  *result = {};
  // Make sure the serialized type is one we know how to deal with, and convert
  // it from a flatbuffer enum into a constant used by the kernel C API.
  TF_LITE_ENSURE_STATUS(ConvertTensorType(flatbuffer_tensor.type(),
...
size_t MicroAllocator::GetDefaultTailUsage(bool is_memory_planner_given) {
  Serial.println("micro_allocator.cpp10");
...
MicroAllocator* MicroAllocator::Create(uint8_t* tensor_arena, size_t arena_size,
                                       MicroMemoryPlanner* memory_planner,
                                       ErrorReporter* error_reporter) {
  Serial.println("micro_allocator.cpp11");
...
MicroAllocator* MicroAllocator::Create(uint8_t* tensor_arena, size_t arena_size,
                                       ErrorReporter* error_reporter) {
  Serial.println("micro_allocator.cpp12");
  uint8_t* aligned_arena =
      AlignPointerUp(tensor_arena, MicroArenaBufferAlignment());
  size_t aligned_arena_size = tensor_arena + arena_size - aligned_arena;
  SimpleMemoryAllocator* memory_allocator = SimpleMemoryAllocator::Create(
...
MicroAllocator* MicroAllocator::Create(SimpleMemoryAllocator* memory_allocator,
                                       MicroMemoryPlanner* memory_planner,
                                       ErrorReporter* error_reporter) {
  Serial.println("micro_allocator.cpp13");
...
SubgraphAllocations* MicroAllocator::StartModelAllocation(const Model* model) {
  Serial.println("micro_allocator.cpp14");
...
TfLiteStatus MicroAllocator::FinishModelAllocation(
    ...
    ScratchBufferHandle** scratch_buffer_handles) {
  Serial.println("micro_allocator.cpp15");
  if (!model_is_allocating_) {
    TF_LITE_REPORT_ERROR(error_reporter_,
...
void* MicroAllocator::AllocatePersistentBuffer(size_t bytes) {
  Serial.println("micro_allocator.cpp16");
...
TfLiteStatus MicroAllocator::RequestScratchBufferInArena(size_t bytes,
                                                         int subgraph_idx,
                                                         int* buffer_idx) {
  Serial.println("micro_allocator.cpp17");
...
TfLiteStatus MicroAllocator::FinishPrepareNodeAllocations(int node_id) {
  Serial.println("micro_allocator.cpp18");
...
size_t MicroAllocator::used_bytes() const {
  Serial.println("micro_allocator.cpp19");
  return memory_allocator_->GetUsedBytes();
}

TfLiteStatus MicroAllocator::AllocateNodeAndRegistrations(
    const Model* model, SubgraphAllocations* subgraph_allocations) {
  Serial.println("micro_allocator.cpp20");
...
TfLiteTensor* MicroAllocator::AllocatePersistentTfLiteTensor(
  ...
  Serial.println("micro_allocator.cpp21");
...
TfLiteTensor* MicroAllocator::AllocateTempTfLiteTensor(
    const Model* model, const SubgraphAllocations* subgraph_allocations,
    int tensor_index, int subgraph_index) {
  Serial.println("micro_allocator.cpp22");
  const SubGraph* subgraph = model->subgraphs()->Get(subgraph_index);
  TFLITE_DCHECK(subgraph != nullptr);
...
void MicroAllocator::ResetTempAllocations() {
  Serial.println("micro_allocator.cpp23");
  memory_allocator_->ResetTempAllocations();
}

TfLiteStatus MicroAllocator::AllocateTfLiteEvalTensors(
    const Model* model, SubgraphAllocations* subgraph_allocations) {
  Serial.println("micro_allocator.cpp24");
  TFLITE_DCHECK(subgraph_allocations != nullptr);
...
TfLiteStatus MicroAllocator::AllocateVariables(const SubGraph* subgraph,
                                               TfLiteEvalTensor* eval_tensors) {
  Serial.println("micro_allocator.cpp25");
...
TfLiteTensor* MicroAllocator::AllocatePersistentTfLiteTensorInternal() {
  Serial.println("micro_allocator.cpp26");
...
TfLiteStatus MicroAllocator::PopulateTfLiteTensorFromFlatbuffer(
  ...
  Serial.println("micro_allocator.cpp27");
...
TfLiteStatus MicroAllocator::CommitStaticMemoryPlan(
  ...
  Serial.println("micro_allocator.cpp28");
  size_t head_usage = 0;
...
TfLiteStatus MicroAllocator::AllocateScratchBufferHandles(
  ...
  Serial.println("micro_allocator.cpp29");
...
TfLiteStatus MicroAllocator::InitScratchBufferData() {
  Serial.println("micro_allocator.cpp30");
...
internal::ScratchBufferRequest* MicroAllocator::GetScratchBufferRequests() {
  Serial.println("micro_allocator.cpp31");
...
BuiltinDataAllocator* MicroAllocator::GetBuiltinDataAllocator() {
  Serial.println("micro_allocator.cpp32");
