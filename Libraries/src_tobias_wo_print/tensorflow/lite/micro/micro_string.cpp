...
char* ReverseStringInPlace(char* start, char* end) {
  //Serial.println("micro_string.cpp1");
  char* p1 = start;
  char* p2 = end - 1;
  while (p1 < p2) {
    char tmp = *p1;
...
char* StrCatStr(char* main, int main_max_length, const char* to_append) {
  //Serial.println("micro_string.cpp2");
  char* current = main;
  while (*current != 0) {
    ++current;
  }
...
char* FastUInt32ToBufferLeft(uint32_t i, char* buffer, int base) {
  //Serial.println("micro_string.cpp3");
  char* start = buffer;
  do {
...
char* FastInt32ToBufferLeft(int32_t i, char* buffer) {
  //Serial.println("micro_string.cpp4");
  uint32_t u = i;
  if (i < 0) {
    *buffer++ = '-';
    u = -u;
...
char* StrCatInt32(char* main, int main_max_length, int32_t number) {
  //Serial.println("micro_string.cpp5");
...
char* StrCatUInt32(char* main, int main_max_length, uint32_t number, int base) {
  //Serial.println("micro_string.cpp6");
  char number_string[kFastToBufferSize];
...
char* FastFloatToBufferLeft(float f, char* buffer) {
  //Serial.println("micro_string.cpp7");
  char* current = buffer;
...
extern "C" int MicroVsnprintf(char* output, int len, const char* format,
                              va_list args) {
  //Serial.println("micro_string.cpp12");
  int output_index = 0;
...
extern "C" int MicroSnprintf(char* output, int len, const char* format, ...) {
  //Serial.println("micro_string.cpp13");
  va_list args;
