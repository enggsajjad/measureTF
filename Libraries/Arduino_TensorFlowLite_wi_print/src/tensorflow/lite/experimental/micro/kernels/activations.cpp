...
inline void ReluQuantized(int32_t lower, const RuntimeShape& input_shape,
                          ...
                          Q* output_data) {
  Serial.println("activations.cpp1");
...
inline void ReluFloat(const RuntimeShape& input_shape, const float* input_data,
  ...
  Serial.println("activations.cpp2");
  const int flat_size = MatchingFlatSize(input_shape, output_shape);
  for (int i = 0; i < flat_size; ++i) {
...
inline void Relu6Float(const RuntimeShape& input_shape, const float* input_data,
                       const RuntimeShape& output_shape, float* output_data) {
  Serial.println("activations.cpp3");
  const int flat_size = MatchingFlatSize(input_shape, output_shape);
  for (int i = 0; i < flat_size; ++i) {
    const float val = input_data[i];
...
inline void Relu6Quantized(Q lower, Q upper, const RuntimeShape& input_shape,
  ...
  Serial.println("activations.cpp4");
  const int flat_size = MatchingFlatSize(input_shape, output_shape);
  for (int i = 0; i < flat_size; ++i) {
    const Q val = input_data[i];
...
TfLiteStatus ReluPrepare(TfLiteContext* context, TfLiteNode* node) {
  Serial.println("activations.cpp5");
  return kTfLiteOk;
}

TfLiteStatus ReluEval(TfLiteContext* context, TfLiteNode* node) {
  Serial.println("activations.cpp6");
...
TfLiteStatus Relu6Prepare(TfLiteContext* context, TfLiteNode* node) {
  Serial.println("activations.cpp7");
  return kTfLiteOk;
}

TfLiteStatus Relu6Eval(TfLiteContext* context, TfLiteNode* node) {
  Serial.println("activations.cpp8");
  const TfLiteTensor* input = GetInput(context, node, kInputTensor);
  TfLiteTensor* output = GetOutput(context, node, kOutputTensor);
...
TfLiteRegistration* Register_RELU() {
  Serial.println("activations.cpp9");
...
TfLiteRegistration* Register_RELU6() {
  Serial.println("activations.cpp10");
  static TfLiteRegistration r = {/*init=*/nullptr,
