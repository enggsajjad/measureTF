...
void ModelExecuter::setModel(const unsigned char *newModel, uint32_t modelLength)
{
  ...
  //serial->println(MEMORY, "kTensorAreaUsed", interpreter->arena_used_bytes());//Sajjad Comment
...
{
  //return interpreter->arena_used_bytes();//Sajjad Comment
}
...
int ModelExecuter::execute()
{
  ...
  //interpreter->GetMicroAllocator().PrintAllocations();//Sajjad Comment
  //interrupts();
  return 0;
}
